...
class WebZip {
  ...
  var $fp;
  var $dirstr='';
  var $exlude;// 添加排除，是一个数组，每一个值就是文件夹的路径
  ...
  并返回文件的写入权限.
  */
	function startfile($path = 'webzip.zip') {
		// 处理路径
  ...
  function process($dir = '.',$root = '.'){
    ...
    while($file = readdir($handle)) {
      //添加排除
      $root = (substr($root,-1) == DIRECTORY_SEPARATOR ? $root : $root.DIRECTORY_SEPARATOR);
      $path = $this->str_replace_first($root,'',$dir.DIRECTORY_SEPARATOR.$file);
      if($file == '.' || $file == '..' || in_array($dir.DIRECTORY_SEPARATOR.$file,$this->exlude))continue;
      if(is_dir($dir.DIRECTORY_SEPARATOR.$file)) {
			  $sub_file_num += $this->process($dir.DIRECTORY_SEPARATOR.$file,$root);
		  }
		  else {
		    if(realpath($this->gzfilename) != realpath($dir.DIRECTORY_SEPARATOR.$file)){
          $this->addfile(implode('',file($dir.DIRECTORY_SEPARATOR.$file)),$path);
			    $sub_file_num ++;
			  }
		  }
		}
		...
		if(!$sub_file_num) {
      $root = (substr($root,-1) == DIRECTORY_SEPARATOR ? $root : $root.DIRECTORY_SEPARATOR);
      $path = $this->str_replace_first($root,'',$dir.DIRECTORY_SEPARATOR);
      if($path)$this->addfile("",$path);
    }
		return $sub_file_num;
	}
...
if($ZIP->startfile('web.zip')) { // startfile的参数就是准备生成的文件的路径，一般是相对路径，使用绝对路径没有问题
  $ZIP->exlude = array('webzip.php'); // exlude指排除，数组中的每一个元素是指要排除的文件或文件夹的路径，这个路径是相对于下方process中的路径来说的，例如下方使用了'.'这样的相对路径，那么这个地方你就应该填写'./exlude'这样的相对值，如果下方使用了'/home/var/www/html/'，那么这里就应该使用'/home/var/www/html/backup/'这样的绝对值，总而言之，这里的路径应该包含下方的第一个参数

...
class DbManage {
     ...
     */
    function backup($tablename = '', $dir = './database/', $size = 2000) {
        $dir = $dir ? $dir : './database/';
        // 创建目录
        if (! is_dir ( $dir )) {
        ...
        if (! empty ( $tablename )) {
            ...
            $this->_showMsg("<span class='imp'>数据库备份成功。</span>");
        } else {
            $this->_showMsg('正在备份数据库...');
            // 备份全部表
            ...
            $this->_showMsg("恭喜您! <span class='imp'>数据库备份成功</span>");
        }
    }

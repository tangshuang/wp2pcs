...
class DBZIP {
    ...
    function __construct($host, $username, $password, $database, $charset = 'utf8') {
        ...
        // 连接数据库
        if(($this->db = @mysql_connect ( $this->host, $this->username, $this->password )) === false) {
          return;
        }
        // 选择使用哪个数据库
        if(mysql_select_db ( $this->database, $this->db ) === false) {
          return;
        }
        // 数据库编码方式
     ...
     */
    function backup($dir,$size = 2000,$tablename = '') {
        // 创建目录
        if (! is_dir ( $dir )) {
        ...
        if (! empty ( $tablename )) {
            ...
            // 备份全部表
            if ($tables = mysql_query ( "show table status from " . $this->database ) === false) {
                return;
            }
            // 插入dump信息
            $sql .= $this->_retrieve ();
    ...
    private function _retrieve() {
        ...
        $value .= '-- Created by DbManage class, Power By WP2PCS. ' . $this->ds;
        $value .= '-- http://www.wp2pcs.com ' . $this->ds;
        $value .= '--' . $this->ds;
    ...
    private function _write_file($sql, $filename, $dir) {
        ...
        $re = true;
        if (! @$fp = fopen ( realpath($dir).DIRECTORY_SEPARATOR.$filename, "w+" )) {
            $re = false;
        }
        if (! @fwrite ( $fp, $sql )) {
            $re = false;

...
function wp_diff_to_pcs_action(){
  ...
  if(!isset($_POST['page'])) {
	...
	// 设置增量备份
	elseif(@$_POST['page'] == @$_GET['page'] && @$_POST['action'] == 'wp_diff_to_pcs_update_options'){
		check_admin_referer();
      ...
      foreach($local_paths as $key => $path) {
        $local_paths[$key] = get_real_path($path);
      }
  ...
  // 开启增量备份定时任务
  elseif(@$_POST['page'] == @$_GET['page'] && @$_POST['action'] == 'wp_diff_to_pcs_open_send_file'){
    update_option('wp_diff_to_pcs_status',1);
    update_option('wp_diff_to_pcs_last_time',0);
    $run_rate = get_option('wp_diff_to_pcs_run_rate');
    wp_schedule_event(time(),$run_rate,'wp_diff_to_pcs_corn_task');
    wp2pcs_clean_redirect('#wp-to-pcs-diff-form');
  }
  // 关闭增量备份
  elseif(@$_POST['page'] == @$_GET['page'] && @$_POST['action'] == 'wp_diff_to_pcs_close_send_file') {
    update_option('wp_diff_to_pcs_status',0);
...
function wp_diff_to_pcs_update_file_list(){
  ...
  foreach($local_files as $key => $local_file) {
    if(strpos($local_file,WP2PCS_TMP_PATH) === 0) {
...
function wp_diff_to_pcs_corn_function(){
	if(get_option('wp_diff_to_pcs_status') != 1)
		return;
  ...
  set_php_ini('timezone');
	...
	if(isset($local_files[$diff_cursor])){
      ...
      update_option('wp_diff_to_pcs_last_time',time());
		...
		if(filemtime($local_file) > $diff_time){
			$file_name = basename($local_file);
			$file_size = get_real_filesize($local_file);
      // 处理一些路径
			$file_path = get_real_path(str_replace_first(ABSPATH,'/',$local_file));
...
function wp_diff_to_pcs_panel(){
  ...
  $btn_option = get_option('wp_diff_to_pcs_status');
  $btn_value = ($btn_option == 1 ? 'wp_diff_to_pcs_close_send_file' : 'wp_diff_to_pcs_open_send_file');
	$btn_text = ($btn_option == 1 ? '已经开启增量备份，现在关闭' : '开启增量备份');
...
<div class="postbox" id="wp-to-pcs-diff-form">
    ...
    $local_files_record = trailing_slash_path(WP2PCS_TMP_PATH,IS_WIN).'local_files.php';
		...
		<p>网站将被备份到网盘目录：<a href="http://pan.baidu.com/disk/home#path=<?php echo urlencode(WP2PCS_REMOTE_ROOT_PATH); ?>" target="_blank"><?php echo WP2PCS_REMOTE_ROOT_PATH; ?></a></p>
		...
		<p>
			...
			<button type="submit" class="button-primary" name="action" value="wp_diff_to_pcs_update_options">更新</button>
			&nbsp;&nbsp;&nbsp;&nbsp;
			<?php endif; ?>
			<button type="submit" class="<?php echo $btn_class; ?>" name="action" value="<?php echo $btn_value; ?>"><?php echo $btn_text; ?></button>
		</p>
		...
		<?php elseif(!is_really_writable($local_files_record)): ?><p style="color:red;"><?php echo trailing_slash_path(WP2PCS_TMP_PATH,IS_WIN); ?>local_files.php这个文件不可写，无法进行增量备份。赋予它可写权限后必须再点击一次更新按钮。</p><?php endif; ?>
